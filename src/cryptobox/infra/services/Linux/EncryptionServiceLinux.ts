...
export class EncryptionServiceLinux implements EncryptionService {
  public mount(volume: Volume, passwordService: PasswordService): void {
    ...
    let command: string = `encfs ${volume.encryptedFolderPath} ${volume.decryptedFolderPath} --standard`;
    command = `${command} --extpass='${pwdCommand}' --idle=${volume.timeToUnmount}`;
    command = `${command} --require-macs -ohard_remove`;
