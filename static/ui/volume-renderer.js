...
function updateMountBtn() {
  var resp = ipcRenderer.sendSync(constants.IPC_IS_MOUNTED, {
    source: source.value,
  });

  if (resp.isMounted) mountBtn.innerText = "UNmount";
  else mountBtn.innerText = "Mount";
}

function checkIfPasswordExist(source) {
  var ret = ipcRenderer.sendSync(constants.IPC_PASSWORD_EXIST, {
    source: source,
  });
